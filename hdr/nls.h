...
struct nlsExtCntryInfo {
  ...
  SYM_MEMB_T(nlsExtCntryInfo, WORD, dateFmt);                 /* order of portions of date
                                   0: mm/dd/yyyy (USA)
                                   1: dd/mm/yyyy (Europe)
                                   2: yyyy/mm/dd (Japan)
                                 */
...
struct nlsPointer {             /* Information of DOS-65-0X is addressed
  ...
  __DOSFAR(VOID)pointer;            /* the pointer to be returned when the subfunction
...
struct nlsDBCS {                /* The internal structure is unknown to me */
  UWORD numEntries;
  AR_MEMB(nlsDBCS, UWORD, dbcsTbl, 4);             /* I don't know max size but it should need
...
/* initially the object rpos is pointing to conforms to a
	struct nlsPackage, where:
	  __DOSFAR(struct nlsPackage)nxt;   is missing
