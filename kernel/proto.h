...
WORD ASMPASCAL INITTEXT _close(WORD fd);
WORD ASMPASCAL INITTEXT _dup2(WORD oldfd, WORD newfd);
ULONG ASMPASCAL INITTEXT _lseek(WORD fd, DWORD position);
UWORD ASMPASCAL INITTEXT allocmem(UWORD size);
...
__FAR(void) ASMPASCAL FAR DetectXMSDriver(VOID);
WORD ASMPASCAL FAR call_XMScall(__FAR(void) driverAddress, UWORD ax, UWORD dx);

void ASMPASCAL INITTEXT init_PSPSet(UWORD psp_seg);
