...
#define KeyboardShiftState() (*(BYTE FAR *)(MK_FP(0x40,0x17)))
  ...
  for (;;)
  {
    ____R(IoReqHdr.r_length) = sizeof(request);
    ____R(IoReqHdr.r_unit) = dpbp->dpb_subunit;
    ...
    switch (mode)
    {
      case DSKWRITE:
        if (verify_ena)
        {
          ____R(IoReqHdr.r_command) = C_OUTVFY;
          break;
        }
        /* else fall through */
      case DSKWRITEINT26:
        ____R(IoReqHdr.r_command) = C_OUTPUT;
        break;

      case DSKREADINT25:
      case DSKREAD:
        ____R(IoReqHdr.r_command) = C_INPUT;
        break;
      default:
        return 0x0100;          /* illegal command */
    }

    ____R(IoReqHdr.r_status) = 0;
    ____R(IoReqHdr.r_meddesc) = dpbp->dpb_mdb;
    ____R(IoReqHdr.r_count) = numblocks;
    if ((dpbp->dpb_device->dh_attr & ATTR_HUGE) || blkno >= MAXSHORT)
    {
      ____R(IoReqHdr.r_start) = HUGECOUNT;
      ____R(IoReqHdr.r_huge) = blkno;
    }
    else
      ____R(IoReqHdr.r_start) = (UWORD)blkno;
    /*
