...
{
  ConvertNameSZToName83(FCBName, get_root(MK_FAR_STR_SCP(PathName)));
...
long DosOpenSft(char FAR * fname, unsigned flags, unsigned attrib)
{
  ...
  if ((sftp = get_free_sft(&sft_idx)) == (sft FAR *)MK_FP(-1, -1))
    return DE_TOOMANY;
...
COUNT CloneHandle(unsigned hndl)
{
  ...
  if (sftp == (sft FAR *)MK_FP(-1, -1) || (sftp->sft_mode & O_NOINHERIT))
    return DE_INVLDHNDL;
...
COUNT DosForceDup(unsigned OldHandle, unsigned NewHandle)
{
  ...
  /* Get the SFT block that contains the SFT                      */
  if ((Sftp = get_sft(OldHandle)) == (sft FAR *)MK_FP(-1, -1))
    return DE_INVLDHNDL;
