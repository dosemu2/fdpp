...
dispatch:
  ...
  */
  ...
  DOS_08:
    ...
    case 0x0a:
  DOS_0A:
      read_line(get_sft_idx(STDIN), FP_DS_DX, TRUE);
      break;
...
*/
    ...
    /* LFN API, support only 0x42 and 0xA6 for now */
    case 0x71:
      switch (lr.AL)
      {
        case 0x42:
...
error_carry:
  SET_CARRY_FLAG();
exit_dispatch:
  n_fmemcpy(r, &lr, sizeof(lregs) - 4); /* copy lr -> r but exclude flags */
  r->DS = lr.DS;
  r->ES = lr.ES;
real_exit:;
...
VOID ASMCFUNC int2F_12_handler(struct int2f12regs FAR *regs)
{
  ...
  switch (r.AL)
  {
    ...
    case 0x12:                 /* get length of asciiz string */
      ...
      r.CX = fstrlen((char FAR *)MK_FP(r.ES, r.DI)) + 1;
    ...
    case 0x1f:
      {
        ...
        ____R(TempCDS.cdsBackslashOffset) = 2;
        if (cdsp->cdsFlags)
        {
          TempCDS.cdsDpb = cdsp->cdsDpb;
          ____R(TempCDS.cdsFlags) = CDSPHYSDRV;    // don't inherit CDS flags
        }
        else
        {
          TempCDS.cdsDpb = NULL;
          ____R(TempCDS.cdsFlags) = 0;
        }
        ____R(TempCDS.cdsStrtClst) = 0xffff;
        ____R(TempCDS.cdsParam) = 0xffff;
        ____R(TempCDS.cdsStoreUData) = 0xffff;
    ...
    case 0x23:                 /* check if character device */
      {
        ...
        dhp = IsDevice(MK_FAR_CSTR_SCP(DirEntBuffer.dir_name));
    ...
    case 0x25:                 /* get length of asciiz string */
      ...
      r.CX = fstrlen((char FAR *)MK_FP(r.DS, r.SI)) + 1;
      break;
